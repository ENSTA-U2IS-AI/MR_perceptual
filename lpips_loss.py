...
loss_fn = mrpl.LPIPS(net='vgg')
if(opt.use_gpu):
    loss_fn.cuda()

ref = mrpl.im2tensor(mrpl.load_image(opt.ref_path))
pred = Variable(mrpl.im2tensor(mrpl.load_image(opt.pred_path)), requires_grad=True)
if(opt.use_gpu):
    with torch.no_grad():
        ref = ref.cuda()
        pred = pred.cuda()
...
ax.imshow(mrpl.tensor2im(ref))
ax.set_title('target')
ax = fig.add_subplot(133)
ax.imshow(mrpl.tensor2im(pred.data))
...
for i in range(1000):
    ...
    if i % 10 == 0:
        ...
        pred.data = torch.clamp(pred.data, -1, 1)
        pred_img = mrpl.tensor2im(pred.data)
