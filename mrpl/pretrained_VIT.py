...
class Block_extra(nn.Module):
    ...
    def forward(self, x, collect_features=False):

        x, feat = self.attn(self.norm1(x), return_attn=True)
...
class Attention_last(nn.Module):
    ...
    def forward(self, x, return_attn=False):
        ...
        attn_a = (q @ k.transpose(-2, -1)) * self.scale

        attn = attn_a.softmax(dim=-1)
        attn = self.attn_drop(attn)
...
class Block_last(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.attn(self.norm1(x),return_attn=True)
        return x
...
'''class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.blocks0 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[0], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks1 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[1], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks2 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[2], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks3 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[3], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks4 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[4], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks5 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[5], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks6 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[6], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks7 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[7], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks8 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[8], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks9 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[9], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks10 = Block_extra(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[10], norm_layer=norm_layer, act_layer=act_layer)
        self.blocks11 = Block_last(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
            attn_drop=attn_drop_rate, drop_path=dpr[11], norm_layer=norm_layer, act_layer=act_layer)
    ...
    def forward_features(self, x):
        ...
        x_cp = x.copy().detach
        list_features.append(x_cp)
        x, feat1 = self.blocks0(x)
        #list_features.append(feat)
        x, feat = self.blocks1(x)
        #list_features.append(feat)
        x, feat = self.blocks2(x)
        #list_features.append(feat)
        x, feat2 = self.blocks3(x)
        #list_features.append(feat)
        x, feat = self.blocks4(x)
        #list_features.append(feat)
        x, feat = self.blocks5(x)
        #list_features.append(feat)
        x, feat3 = self.blocks6(x)
        #list_features.append(feat)
        x, feat = self.blocks7(x)
        #list_features.append(feat)
        x, feat4 = self.blocks8(x)
        #list_features.append(feat)
        x, feat = self.blocks9(x)
        #list_features.append(feat)
        x, feat = self.blocks10(x)
        #list_features.append(feat)
        x, feat5 = self.blocks11(x)
        #list_features.append(feat)
        VIT_outputs = namedtuple("VITOutputs", ['relu1','relu2','relu3','relu4','relu5'])
        out = VIT_outputs(feat1,feat2,feat3,feat4,feat5)
...
@register_model
def vit_base_patch16_224_miil_in21k(pretrained=False, requires_grad=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
...
@register_model
def vit_base_patch16_224_miil(pretrained=False, requires_grad=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    Weights taken from: https://github.com/Alibaba-MIIL/ImageNet21K
    """
    pretrained = False
...
class VIT_feature(torch.nn.Module):
    def __init__(self, requires_grad=False, pretrained=True):
        super(VIT_feature, self).__init__()
        VIT_featurepretrained_features = list( vit_base_patch16_224_miil(pretrained=pretrained).children())
        VIT_featurepretrained_attention_features = list( VIT_featurepretrained_features[2].children())
        ...
        self.N_slices = 5
        for x in range(2):
            self.slice1.add_module(str(x), VIT_featurepretrained_features[x])

        for x in range(0, 5):
            self.slice2.add_module(str(x), VIT_featurepretrained_attention_features[x])
        for x in range(5, 8):
            self.slice3.add_module(str(x), VIT_featurepretrained_attention_features[x])
        for x in range(8, 10):
            self.slice4.add_module(str(x), VIT_featurepretrained_attention_features[x])
        for x in range(10, 12):
            self.slice5.add_module(str(x), VIT_featurepretrained_attention_features[x])
        if not requires_grad:
    ...
    def forward(self, X):
        ...
        (N, _, _, _) = X.size()
        h = self.slice1(X)
        h_relu1 = torch.reshape(h,(N,14,14,768))
        h = self.slice2(h)
        h_relu2 = torch.reshape(h,(N,14,14,768))
        h = self.slice3(h)
        h_relu3 = torch.reshape(h,(N,14,14,768))
        h = self.slice4(h)
        h_relu4 = torch.reshape(h,(N,14,14,768))
        h = self.slice5(h)
        h_relu5 = torch.reshape(h,(N,14,14,768))
        VIT_outputs = namedtuple("VITOutputs", ['relu1','relu2','relu3','relu4','relu5'])
        out = VIT_outputs(h_relu1, h_relu2, h_relu3, h_relu4, h_relu5)
